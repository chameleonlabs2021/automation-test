...
class DateCalc {
        ...
        if (durationUnit && typeof durationUnit === 'object') {
            ...
            switch (this.#durationUnit) {
                ...
                case 'wholeDays':
                    ...
                    break;
                case 'hours':
                    this.#getDuration = this.hourCount;
                    this.#useAnchorForDurationCalc = false;
                    break;
                case 'ms':
                    ...
                    break;
                default:
                    throw new Error(`Duration unit ${durationUnit} not supported!`);
            }
        }
    }
        ...
        if (timestamp < this.#anchorTimestamp) {
            ...
            switch (this.#durationUnit) {
                ...
                case 'socotraMonths':
                    ...
                    break;
                default:
                    throw new Error(`Duration unit for base adjust ${this.#durationUnit} not supported.`);
            }
        }
    }
    /* one based months */
    getTimestamp(year = 1970, month = 1, day = 1, hour = 0,
        ...
        if (month < 1 || month > 12) {
            throw new Error(`getTimestamp: month ${month} out of range. Use 1-based month values.`);
        }
     ...
     *                   Default is true
    //  preferredBaseDayTimestamp: if specified, the base day of the month will be set to this timestamp
     *
    ...
    getDateSequence(startTimestamp, endTimestamp, options = null) {
        ...
        const preferredBaseDayTimestamp = options.preferredBaseDayTimestamp;
        const preferredLastDayOfMonth = options.preferredLastDayOfMonth;
        const preventStartAndEndTimestampOverlap = options.preventStartAndEndTimestampOverlap !== false;

        const cursor = this.toMoment(
            preventStartAndEndTimestampOverlap && anchorTimestamp === endTimestamp ? anchorTimestamp + 1 : anchorTimestamp
        );

        let baseDay = cursor.date();
        let sequence = [];
        let fractionedItems = [];
        ...
        if (Array.isArray(increment)) {
            if (startTimestamp !== anchorTimestamp) {
                throw new Error("Must have startTimestamp equal anchorTimestamp when using day arrays.");
            }
            ...
            if (startTimestamp !== anchorTimestamp) {
                throw new Error("Must have startTimestamp equal anchorTimestamp when using eon interval");
            }
        }
            ...
            while (startTimestamp < cursor.valueOf()) {
                switch (increment) {
                    ...
                    case '2week':
                        cursor.add(-14, 'days');
                        break;
                    // Added for testing purposes. Added by Synpulse8
                    case '12day':
                        cursor.add(-12, 'days');
                        break;
                    case 'day':
                    ...
                    default:
                        throw new Error(`Span increment '${increment}' not supported!`);
                }
            }
        }
        ...
        let i = 0;
        let count = 0;
        let isPreferredDayProcessed = false;
        while (cursor.valueOf() < endTimestamp) {
            // Handle effective preferred payment date. Added by Synpulse8
            // this.incrementMonth() < options.preferredBaseDayTimestamp ? options.preferredBaseDayTimestamp : this.incrementMonth(options.preferredBaseDayTimestamp)
            if (!isPreferredDayProcessed && preferredBaseDayTimestamp && cursor.valueOf() >= preferredBaseDayTimestamp) {
                const preferredBaseDate = this.toMoment(preferredBaseDayTimestamp);
                let newBaseDay = preferredBaseDate.date();
                console.log(`Previous base day is ${baseDay} and new base day is ${newBaseDay}`);
                // Adjust preferred payment date to last day of month if needed
                if (newBaseDay >= preferredLastDayOfMonth) {
                    console.log("preferred payment date is last day of month");
                    newBaseDay = 31;
                }

                let incrementMonth = newBaseDay > baseDay ? -1 : 0;
                baseDay = newBaseDay;
                this.incrementMomentByMonth(cursor, baseDay, incrementMonth);
                isPreferredDayProcessed = !isPreferredDayProcessed;
                fractionedItems.push(sequence[sequence.length - 1], cursor.valueOf());
                console.log("fractionedItems", fractionedItems);
            }
        ...
        if (returnIntervals) {
            ...
            for (let i = 1; i < sequence.length; i++) {
                sequence[i - 1].endTimestamp = preventStartAndEndTimestampOverlap ? sequence[i].startTimestamp - 1 : sequence[i].startTimestamp;
            }
            sequence[sequence.length - 1].endTimestamp = endTimestamp;
        }
        const overlapAdjustment = preventStartAndEndTimestampOverlap ? 1 : 0;
        if (returnMetadata) {
            return { sequence, startCursor, endCursor: cursor.valueOf() - overlapAdjustment, fractionedItems, baseDay };
        }
        else {
            return sequence;
        }
    ...
    getNewDayOfMonthTimestamp(timestamp, dayOfMonth) {
        const m = this.toMoment(timestamp);
        m.date(dayOfMonth);
        return m.valueOf();
    }
    ...
    getStartOfDayTimestamp(timestamp) {
        return this.toMoment(timestamp).startOf('day')
            .valueOf();
    }
    ...
    getMidDayTimestamp(timestamp) {
        return this.toMoment(timestamp).startOf('day').add(12, 'hours')
            .valueOf();
    }
    ...
    hourCount(startTimestamp, endTimestamp) {
        const startM = this.toMoment(startTimestamp);
        const endM = this.toMoment(endTimestamp);

        return endM.diff(startM, 'hours');
    }

    #getRatioCalculation(startTimestamp, splitTimestamp, endTimestamp, getRatioFn, clampZeroToOne = true) {
        if (startTimestamp >= endTimestamp) {
            throw new Error("Duration Ratio: Denominator cannot be zero or negative.");
        }

        if (splitTimestamp > endTimestamp) {
            throw new Error("Duration Ratio: Split cannot be after end.");
        }
